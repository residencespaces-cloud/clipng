...
export function ClipperCampaigns({
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4">
      {campaigns.map((c) => {
        const rules = rulesFromCampaign(c);
        const showRules = hasCampaignRules(rules);
          ...
          <div key={c.id} className="bg-card border border-border rounded-xl p-5 space-y-4">
            ...
            {showRules && (
              <div className="rounded-lg border border-border bg-secondary/40 p-3 space-y-1.5">
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Creator rules</p>
              ...
              <div className="bg-secondary rounded-xl p-4 space-y-3">
                ...
                {showRules && (
                  <div className="rounded-lg border border-border bg-background/50 p-3 space-y-1.5">
                    <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Follow these rules</p>
                    <CampaignRulesPanel rules={rules} compact />
                  </div>
                )}
                  ...
                  {showRules && (
                    <label className="flex items-start gap-2 text-xs text-foreground cursor-pointer">
                      <input
                        type="checkbox"
                        checked={rulesAcked}
                        onChange={(e) => onRulesAcked(e.target.checked)}
                        className="mt-0.5 accent-primary"
                      />
                      <span>I read and will follow the creator rules for this campaign.</span>
                    </label>
                  )}
                  <label className="flex items-start gap-2 text-xs text-foreground cursor-pointer">
                    <input
